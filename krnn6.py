...
#        self.embed.data.normal_()
        ...
        self.embed1 = nn.Parameter(torch.FloatTensor(16,num_timesteps_output))
    ...
    def forward(self, A, X):
        ...
        sz = X.size()
        out = []
        query=X.contiguous().view(-1, sz[2], sz[3]).permute(0, 2, 1)
        query=torch.rfft(query,1)
        query=query.permute(0,2,3,1)
        szz=query.size()
                
        query=query.contiguous().view(sz[0], -1) #(32*207,time,2)
        query=query.contiguous().view(sz[0],sz[1], -1)    #(32,207,time*2)    
#        print('query:',query.size()) 
        ...
        for i in range(len(self.encoders)):
            ...
            h_val2= self.dila_conv1[i](X)
            filter = torch.tanh(h_val)           
            gate = torch.sigmoid(h_val2)
            h_val=filter * gate
